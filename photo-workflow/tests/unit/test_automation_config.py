...
def test_calibrated_prediction_thresholds_policy_1_3() -> None:
    """3a: Kalibrierte Prognose-Schwellen in config/config.yaml (Policy 1.3)."""
    from pathlib import Path
    import yaml

    cfg = yaml.safe_load(Path("config/config.yaml").read_text(encoding="utf-8"))
    auto = cfg["automation"]
    assert auto["policy_version"] == "1.3"
