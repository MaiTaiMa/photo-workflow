...
def _row(execution=None):
    return {"file": "a.jpg", "family_tags": ["family:test"], "family_regions": [], "execution": execution or {"target_relative_path": "_Review/a.jpg", "moved": False, "family_metadata_written": False, "culling_metadata_written": False}}
...
def test_resumes_after_persisted_move(tmp_path):
    ...
    result, updates, _, moved = _run(tmp_path, _row({"target_relative_path": "_Review/a.jpg", "moved": True, "family_metadata_written": False, "culling_metadata_written": False}), _workunit("in_progress"))
    assert result.state == "completed"
    assert moved == []
