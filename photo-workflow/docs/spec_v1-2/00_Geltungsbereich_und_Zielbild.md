...
## 4.1 Automationsmodi (verbindlich)

Das System unterstützt sechs Automationsmodi für den KI-Entscheidungsassistenten:

| Modus | Prediction | Finale Bildentscheidung | Phase-2-Start | Wirkung |
|---|---|---|---|---|
| `off` | keine operative Prediction | bestehende deterministische Regeln | nur manuell | keine KI-Assistenz |
| `shadow` | Diagnose persistenzsicher dokumentiert | unverändert | nur manuell | Lernen und Messung |
...
| `auto_phase2` | wie `auto_phase1` | wie `auto_phase1` | nur bei `automatic_handoff` + allen Phase-2-Gates | lokale automatische Phase 1 und 2 |
| `full_auto` | wie `auto_phase2` | wie `auto_phase2` | nur bei `automatic_handoff` + allen Phase-2-Gates | vollständiger lokaler Lauf bis `04_TEMP_FINAL` |

`shadow` und `assisted` dürfen **niemals** `keep`/`review`/`reject`, Dateimoves, Archivierung, ARW-Aktionen oder States verändern. Jeder operative Modus muss explizit in der Config aktiviert sein.
